...
namespace webwork {
    std::string Cookie::ToString() const {
        ...
        if (domain.empty()) attributes.emplace_back("Domain=" + domain);
        if (expires.has_value()) {
            const std::chrono::zoned_time expiresGMT{"Etc/GMT", expires.value()};
            const int seconds = std::stoi(std::format("{0:%S}", expires.value()));
            attributes.emplace_back(std::format("Expires={0:%a}, {0:%d} {0:%m} {0:%Y} {0:%H}:{0:%M}:{1} GMT", expiresGMT, seconds));
        }
        ...
        if (path.empty()) attributes.emplace_back("Path=" + path);
